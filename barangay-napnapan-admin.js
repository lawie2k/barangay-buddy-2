  ...
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      div.innerHTML = `
        ...
        <button class="accept-btn" data-email="${data.email}" data-name="${data.firstName}" data-doc-id="${doc.id}">Send Email</button>
        <span class="accepted-msg" style="display:${data.emailSent ? "inline" : "none"}; color: green; font-weight: bold;">✔ Email Sent</span>
      `;
...
/*-----------------------------resident records------------------------------*/
document.addEventListener("DOMContentLoaded", async () => {
  const residentRecordsContainer = document.querySelector(".resident-rec-napnapan");
...
async function updateResidentSummary() {
  try {
    ...
    snapshot.forEach((doc) => {
      ...
      if (data.registeredVoters === true || data.registeredVoters === 'yes' || data.registeredVoters === 'Yes') {
        voterCount++;
      }
    });

    // Calculate percentages
    const malePercentage = totalResidents > 0 ? Math.round((maleCount / totalResidents) * 100) : 0;
    const femalePercentage = totalResidents > 0 ? Math.round((femaleCount / totalResidents) * 100) : 0;
    const voterPercentage = totalResidents > 0 ? Math.round((voterCount / totalResidents) * 100) : 0;
    const populationPercentage = 100; // This is always 100% as it represents the total

    // Update the dashboard elements
    document.querySelector('.resident-items:nth-child(1) .circle h3').textContent = `${populationPercentage}%`;
    document.querySelector('.resident-items:nth-child(2) .circle h3').textContent = `${malePercentage}%`;
    document.querySelector('.resident-items:nth-child(3) .circle h3').textContent = `${voterPercentage}%`;
    document.querySelector('.resident-items:nth-child(4) .circle h3').textContent = `${femalePercentage}%`;
...
const freedomWallContainers = document.querySelectorAll(".freedomWallContainer");
